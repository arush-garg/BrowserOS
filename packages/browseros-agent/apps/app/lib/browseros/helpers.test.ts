...
describe('getAgentServerUrl — VITE_BROWSEROS_SERVER_PORT override', () => {
  let originalChrome: typeof globalThis.chrome | undefined
  ...
  afterEach(() => {
    if (originalChrome) Object.assign(globalThis, { chrome: originalChrome })
  ...
  it('prefers VITE_BROWSEROS_SERVER_PORT over profile prefs in dev', async () => {
    ...
    mock.module('./env', 'VITE_BROWSEROS_SERVER_PORT', '9105')
    mock.module('./prefs', () => ({
      BROWSEROS_PREFS: { MCP_PORT: MCP_PORT_PREF },
    }))
    mock.module('./adapter', () => ({
      BrowserOSAdapter: {
        getInstance: () => ({
    ...
    try {
      const { getAgentServerUrl } = await import('./helpers')
      await expect(getAgentServerUrl()).resolves.toBe('http://127.0.0.1:9105')
      expect(prefRequests).not.toContain(MCP_PORT_PREF)
    } finally {
      if (previousEnvPort !== undefined)
        import.meta.env.VITE_BROWSEROS_SERVER_PORT = previousEnvPort
      else Reflect.deleteProperty(import.meta.env, 'VITE_BROWSEROS_SERVER_PORT')
    }
  })
})
